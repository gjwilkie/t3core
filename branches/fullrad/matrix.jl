module matrix
using input: Nrad, Nv, rgrid_gs2, tracespecs, Tashfac, nedge, deltat, m_trace, Z_trace, DTmix, semianalytic_on, rgrid_in, spline_k, initial_dist, initial_dist_file
...
function init_collop()
  ...
  for ir in 1:Nrad
    ...
    tempMatrix= diagm(vec(nu_s_v3[ir,:])) + (0.5*(diagm(v.*vec(nu_par_v3[ir,:])))*ddv)
...
#  RHS[1] = 0.0
  ...
  matrix[1:Nv,Nv+1] = sinkdist[1:Nv]

  scalefactor = reaction_rate[ir]/(dot(d3v,sinkdist))
...
#  RHS[Nv+1] = RHS[Nv+1]/nedge
  ...
  F0local = matrix\RHS
#  print("Integral = ")
#  println(dot(F0local[1:Nv],d3v))
#  print("nlocal = ")
#  println(nlocal)
#   print("Scale factor = ")
#   println(F0local[end]/scalefactor)
