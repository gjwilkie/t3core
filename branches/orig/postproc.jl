...
using input: Nv,Nrad,Nt, deltat, diffmodel, a, tracespecs, m_trace, ir_sample, rgrid_gs2, rhostar, mref, rmaj, vmax, DTmix, Z_trace, Te_in, Ti_in, ne_in, rgrid_in, nedge, turbfac, Tashfac
...
#    vt_temp = 2.0*sqrt(2.0*Ti[ir]*Tashfac/m_trace)
    vt_temp = 1.0*sqrt(2.0*Ti[ir]*Tashfac/m_trace)
...
#    analheating[ir],err = quadgk(heat_integrand,0.0,vmax)
  ...
  print("Heat flux in = ")
  println(dot(fluxin.*energy,d3v)*Vprime[1])
  #print("Energy generated at 3.5 MeV = ")
  #println(dot(reaction_rate,Vprime)*(rgrid[2]-rgrid[1])*Ealpha)
  print("Energy generated by source = ")
  println(dot(sourceenergy,Vprime)*(rgrid[2]-rgrid[1]))
  print("Energy lost by collisions = ")
  println(dot(totheating,Vprime)*(rgrid[2]-rgrid[1]))
  print("Heat flux out = ")
  println(hflux[end]*Vprime[end])
  print("Turbulent heating of alphas = ")
  println(dot(turbheating,Vprime)*(rgrid[2]-rgrid[1]))
